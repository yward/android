...
class DownloaderImpl(
    ...
    private fun onStartDownload(uuid: UUID, request: Request) {
        ...
        runner.postTask(
            ...
            onResult = { result -> registry.complete(uuid, result); registry.startNext() },
            onError = { registry.complete(uuid, false); registry.startNext() }
        )
    }

    private fun createDownloadTask(request: Request): TaskFunction<Boolean, Int> {
        return if (request.test) {
            this::testDownloadTask
        } else {
            val downloadTask = taskFactory.create()
            val wrapper: TaskFunction<Boolean, Int> = { progress: ((Int) -> Unit), isCancelled ->
     ...
     */
    private fun testDownloadTask(onProgress: OnProgressCallback<Int>, isCancelled: IsCancelled): Boolean {
